...
class PlayerBot(Bot):
    def play_round(self):
        ...
        #### Chanelling all the generated waste to the platform
        if self.player.role_own == 'CH' and self.player.id_in_group == 1:
            Bal = self.player.participant.balance  # balance, storage and capacity before actions
            Store = self.player.participant.store
            Capac = self.player.participant.capac
            yield UniversalDays, dict(actionBCH=Constants.g, priceCH=cu(5))
            yield CHSellDays, dict(actionRESell=Constants.g)
            yield Results
            expect(Bal + self.player.payoff, self.player.participant.balance)  
            expect(Store + self.player.bought, self.player.participant.store)  # same for storage
            expect(Capac - self.player.bought, self.player.participant.capac)  # same for capacity
        ...
        if self.player.role_own == 'UC' and self.player.id_in_group == 2:
            ...
            Store = self.player.participant.store
            Capac = self.player.participant.capac
            yield UniversalDays, dict(actionSUC=0, actionPP=Constants.g, priceUC=cu(5), actionD=0)
            yield Results
            expect(Bal + self.player.payoff, self.player.participant.balance)
            expect(self.player.actionSUC, self.player.participant.store)  # same for storage
            expect(Capac - self.player.actionSUC, self.player.participant.capac)  # same for capacity

        if self.player.role_own == 'CH' and self.player.id_in_group == 3:
            Bal = self.player.participant.balance  # balance, storage and capacity before actions
            Store = self.player.participant.store
            Capac = self.player.participant.capac
            yield UniversalDays, dict(actionBCH=Constants.g, priceCH=cu(6))
            yield CHSellDays, dict(actionRESell=Constants.g)
            yield Results
            expect(Bal + self.player.payoff, self.player.participant.balance)  # proper computation of the balance as per the payoff and the initial balance
            expect(Store + self.player.bought, self.player.participant.store)  # same for storage
        ...
        if self.player.role_own == 'UC' and self.player.id_in_group == 4:
            ...
            Store = self.player.participant.store
            Capac = self.player.participant.capac
            yield UniversalDays, dict(actionSUC=0, actionPP=Constants.g, priceUC=cu(4), actionD=0)
            yield Results
            expect(Bal + self.player.payoff, self.player.participant.balance)  # proper computation of the balance as per the payoff and the initial balance
            expect(self.player.actionSUC, self.player.participant.store)  # same for storage
            expect(Capac - self.player.actionSUC, self.player.participant.capac)  # same for capacity