...
class PlayerBot(Bot):
    def play_round(self):
        ...
        if self.player.role_own == 'UC' and self.player.id_in_group == 2:
            yield Days, dict(actionUCBuyBool='Yes', actionSUC=1, actionPP=4, priceUC=cu(5), actionD=0)
            yield Results
        if self.player.role_own == 'UC' and self.player.id_in_group == 4:
            yield Days, dict(actionUCBuyBool='Yes', actionSUC=2, actionPP=3, priceUC=cu(5), actionD=0)
            yield Results