...
class Player(BasePlayer):  
    ...
    actionS = models.IntegerField(min=0, max=Constants.g, label="How many items do you want to store?")
...
class Days(Page):
    form_model = 'player'
    # form_fields = ['actionS', 'actionPP', 'actionD', 'actionFwd', 'actionRESell', 'WstType']  # the action set

    @staticmethod
    def UC_error_message(player, actions):
...
def creating_session(subsession):
    ...
    for player in players:
        if subsession.round_number == 1 and player.role == Constants.UC_role or player.role == Constants.CH_role:
            ...
            player.participant.capac = Constants.Cmax - prev_player.participant.capac  # this is to maintain the capacity over all rounds
